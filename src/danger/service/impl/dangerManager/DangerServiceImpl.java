	...
	public boolean deleteDangerById(Integer id) throws Exception {
		...
		if (sidingList.size() > 0) {
			for (Sidingbiao sidingbiao : sidingList) {
				Integer sidingid = sidingbiao.getSidingid();// 获取四定id
				// 4.2根据四定编号删除四定延期表信息
				...
				// 执行
				List<Delay> sidingDelayList = delayMapper.selectByExample(delayExample);// 根据四定id查询出所有符合条件的四定延期表信息
				if (sidingDelayList.size() > 0) {
					// 根据四定延期表对象删除四定延期表信息
					int countdelay = delayMapper.deleteByExample(delayExample);
				} // for的括号
					// 4.2.1根据四定编号删除隐患跟踪表信息
					// 01根据四定编号查询出所有隐患跟踪id
